...
public class CombinationSumII {
    public List<List<Integer>> combinationSum2(int[] candidates, int target) {
        ...
        combinationSumIIProcess(sortedUniqueCandidates, candidatesTimes, 0, 0,
                target, new LinkedList<Integer>(), result);
     ...
     * @param candidatesTimes 候选数出现的次数，candidatesTimes[i]为sortedUniqueCandidates[i]的个数
     * @param index 当前候选数的下标
     * @param count 当前候选数sortedUniqueCandidates[index]使用过的次数，当count == candidatesTimes[index]时，切换到下一个数
     * @param target 和
     * @param list 当前已选的数列表
     * @param result list结果集合
     */
    public void combinationSumIIProcess(int[] sortedUniqueCandidates, int[] candidatesTimes, int index,
            int count, int target, List<Integer> list, List<List<Integer>> result) {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(combinationSumII.combinationSum2(new int[]{29,19,14,33,11,5,9,23,23,33,12,
                9,25,25,12,21,14,11,20,30,17,19,5,6,6,5,5,11,12,25,31,28,31,33,27,7,33,31,17,13,21,24,
                17,12,6,16,20,16,22,5}, 28));
    }
}
